...
OUTPUT = ROOT / "output"
REPORT_PATH = OUTPUT / "Prismora_Enemies_V1_upload.json"
ENEMIES = ("Scheggino", "Rapido", "Gonfio", "Tessitore", "Guscione", "Monolite")


async def upload_enemies():
    ...
    async with AssetsUploadClient(creator=creator, oauth2_token=oauth_client.token_data["access_token"]) as client:
        for enemy_name in ENEMIES:
            fbx_path = OUTPUT / f"Prismora_{enemy_name}_V1.fbx"
            if not fbx_path.exists():
                raise FileNotFoundError(fbx_path)
            operation = await client.upload_asset_and_wait_for_done_async(
                asset_type=AssetType.MODEL,
                asset_name=f"Prismora {enemy_name} Enemy V1",
                asset_description="Original Prismora Hero Rush enemy, modelled in Blender and optimized for mobile",
                file_path=str(fbx_path),
                upload_request_timeout_seconds=45,
                num_poll_status_tries=18,
                poll_status_request_timeout_seconds=8,
            )
            report = {"name": enemy_name, "ok": bool(operation.done and operation.response and not operation.error)}
            if operation.error:
                report.update(error=operation.error.message, error_code=operation.error.code)
            elif operation.done and operation.response:
                report.update(asset_id=int(operation.response.asset_id), revision_id=int(operation.response.revision_id))
            else:
                report["error"] = "Upload did not finish before polling timed out"
            reports.append(report)
            print("PRISMORA_ENEMY_UPLOAD_ITEM", json.dumps(report))
...
try:
    result = asyncio.run(upload_enemies())
    print("PRISMORA_ENEMY_UPLOAD", json.dumps(result))
except Exception as exc:
    failure = {"ok": False, "fatal": f"{type(exc).__name__}: {exc}"}
    REPORT_PATH.write_text(json.dumps(failure, indent=2), encoding="utf-8")
    print("PRISMORA_ENEMY_UPLOAD_FATAL", json.dumps(failure))
    raise
